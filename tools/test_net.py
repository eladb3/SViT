    ...
    for cur_iter, (inputs, labels, video_idx, meta) in enumerate(test_loader):
        ...
        if cfg.DETECTION.ENABLE:
            ...
            if isinstance(preds, tuple):
                ...
                extra_preds = None
            if isinstance(preds, dict) and cfg.TEST.DATASET == 'ego4d_oscc':
                preds = preds['cls']
            ...
            # a, b = model([inputs[0].transpose(1,2).flatten(0,1)],  {'ordered_t_idx':True})
            # boxes = b['pred_bboxes'].reshape(inputs[0].size(0), 16, 16, 4, 5)[0, range(16), range(16), :, 1:]
            # vsave = misc.get_vsave(); vid = inputs[0][0]; vsave(vid, boxes)
            if isinstance(labels, (dict,)): # vsave = misc.get_vsave(); vid = inputs[0][0]; boxes = extra_preds['pred_bboxes'][0,:,:, 1:]; vsave(vid, boxes)
                ...
                # Update and log stats.
                test_meter.update_stats(
                    preds.detach(), labels.detach(), video_idx.detach(), metadata=meta, extra_preds=extra_preds,
                )
            test_meter.log_iter_stats(cur_iter)
