...
def attention_pool(tensor, pool, thw_shape,  has_cls_embed=True, norm=None):
    ...
    L = T*H*W
    assert O > 0
    tensor, obj = tensor[:, :, :L], tensor[:, :, -O:] # [B, N, THW, d], [B, N, O, d]
    ...
    tensor = tensor.reshape(B, N, C, L_pooled).transpose(2, 3) # [B, N, T'H'W', d]

    if isinstance(pool, nn.Conv3d):
        kT, kH, kW = pool.weight.shape[-3:]
        obj = (
            obj.reshape(B * N * O, 1, 1, 1, C).expand(B * N * O, kT, kH, kW, C).permute(0, 4, 1, 2, 3).contiguous()
        )
        obj = pool(obj) # [B*N*O,  C', 1, 1, 1]
        # assert all([obj.shape[i] == 1 for i in [-1,-2,-3]])
        obj = obj.mean([-1,-2,-3])
        obj = obj.reshape(B, N, O, -1)

    tensor = torch.cat((tensor, obj), dim=2) # [B, N, T'H'W' + O, C']
    if has_cls_embed:
...
def cal_rel_pos_spatial(
    ...
    q_sp_end_idx = sp_idx + (q_t * q_h * q_w)
    k_sp_end_idx = sp_idx + (k_t * k_h * k_w)
    ...
    r_q = q[:, :, sp_idx:q_sp_end_idx].reshape(B, n_head, q_t, q_h, q_w, dim)
    ...
    attn[:, :, sp_idx:q_sp_end_idx, sp_idx:k_sp_end_idx] = (
        attn[:, :, sp_idx:q_sp_end_idx, sp_idx:k_sp_end_idx].view(B, -1, q_t, q_h, q_w, k_t, k_h, k_w)
        + rel_h_q[:, :, :, :, :, None, :, None]
        + rel_w_q[:, :, :, :, :, None, None, :]
