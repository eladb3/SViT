...
def check_nan_losses(loss, extra_msg=None):
    """
    ...
    """
    if math.isnan(loss):
        msg = "ERROR: Got NaN losses {}".format(datetime.now())
        if extra_msg is not None:
            msg = msg + f"extra_msg: {extra_msg}"
...
def _get_model_analysis_input(cfg, use_train_input):
    ...
    if cfg.NUM_GPUS:
        bbox = bbox.cuda()
    md = {'haog_bboxes': bbox[:, -4:].reshape(1, 1,1,4).expand(1,cfg.DATA.NUM_FRAMES,cfg.SVIT.O,4)}
...
def iter_to_cuda(d, _do_check=False, non_blocking = True):
    iter = []
    if isinstance(d, torch.Tensor):
        if _do_check:
            if str(d.device) != 'cpu':
                assert str(d.device.index) == os.environ['LOCAL_RANK']
        return d.cuda(non_blocking=non_blocking)
...
def get_lambdas_dict(cfg):
    ret = dict([
        ["loss_ce", 1],
        ["boxes_l1_loss", 5 * cfg.SVIT.LAMBDA_NODES],
        ["boxes_bce_loss", 1 * cfg.SVIT.LAMBDA_NODES], 
        ["boxes_giou_loss", 2 * cfg.SVIT.LAMBDA_NODES],
        ["loss_contact_state", cfg.SVIT.LAMBDA_EDGES],
        # ["safety_loss", 1], # avoid gradient issues during video & image training
    ])
    if cfg.TRAIN.FORWARD_VIDEO_FRAMES:
        ret['video_image_boxes_l1_loss'] = cfg.SVIT.LAMBDA_CON
    return ret
