...
def topks_correct(preds, labels, ks, after_activation=False):
    """
    ...
    if preds.ndim == 1:
        ...
        if after_activation:
            _value = 0.5
        preds = torch.stack([torch.full_like(preds, _value, device=preds.device, dtype=preds.dtype), preds], dim=-1) # [B, 2]
        ks = [1, 1]

    if preds.size(1) == 0:
        return [0 for k in ks]
