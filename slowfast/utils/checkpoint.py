...
def get_last_mod5_checkpoint(path_to_job):
    """
    Get the last checkpoint from the checkpointing folder.
    Args:
        path_to_job (string): the path to the folder of the current job.
    ...
    names = pathmgr.ls(d) if pathmgr.exists(d) else []
    names = [f for f in names if "checkpoint" in f]
    names = [x for x in names if int(x.rstrip(".pyth").split("_")[-1])%5==0] # checkpoint_epoch_00018.pyth
    assert len(names), "No checkpoints found in '{}'.".format(d)
    # Sort the checkpoints by epoch.
    name = sorted(names)[-1]
    return os.path.join(d, name)
...
def load_checkpoint(
    ...
    if convert_from_caffe2:
        ...
        if inflation:
            ...
            if 'MVIT_AVA' in path_to_checkpoint:
                should_split_qkv=True
            if should_split_qkv:
...
import subprocess
def get_val_epoch(cfg, epoch):
    try:
        plog = os.listdir(os.path.join(cfg.OUTPUT_DIR, 'stdout.log'))
        result = subprocess.run(["grep", "--text", f'val_epoch.*"epoch": "{epoch}.*"', plog], stdout=subprocess.PIPE)
        if result.returncode != 0:
            return {}
        result = result.stdout.decode('utf-8').strip()
        d = eval(result.split("\n")[0].split("json_stats: ")[-1])
...
def split_qkv(d, _keep_qkv=True):
    out = OrderedDict()
    names = ['q', 'k', 'v']
    for k, v in d.items():
        if _keep_qkv: out[k] = v
        if 'qkv' in k:
            # split
