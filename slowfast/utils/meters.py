...
class AVAMeter(object):
    ...
    def finalize_metrics(self, log=True, ks=None):
        """
...
class TestMeter(object):
    ...
    def update_stats(self, preds, labels, clip_ids,metadata=None, extra_preds=None):
        """
...
class TrainMeter(object):
    ...
    def __init__(self, epoch_iters, cfg):
        ...
        self.multi_loss = MultiLossMeter(cfg.LOG_PERIOD)
    ...
    def data_toc(self):
        self.data_timer.pause()
        self.net_timer.reset()
    def update_stats(self, lr, mb_size, dloss=None, is_video=True):
        """
        Update the current stats.
        Args:
        ...
        """
        dloss, _ = prepare_loss_dict(dloss)
...
class ValMeter(object):
    ...
    def update_predictions(self, preds, labels, metadata=None, extra_preds=None):
        """
...
def eval_extra_metrics(cfg, preds, extra_preds, labels, metadata):
    ...
    if isinstance(loss_fun, nn.Module): loss_fun.eval()
    if hasattr(loss_fun, 'forward') and 'extra_preds' in loss_fun.forward.__code__.co_varnames:
        loss_dict = loss_fun(preds, extra_preds, labels, metadata)
        wloss = misc.get_lambdas_dict(cfg)
        loss = sum([wloss[k] * loss_dict[k] for k in loss_dict if k in wloss])
        loss_dict['loss'] = loss
    else:
