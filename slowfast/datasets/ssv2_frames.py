...
from slowfast.datasets.ssv2_MF.autoaugment_torchvision import rand_augment_transform as rand_augment_transform_torchvision
...
class Ssv2_frames(torch.utils.data.Dataset):
    ...
    def __init__(self, cfg, mode, num_retries=10):
        ...
        """
        self.data_root = cfg.SSV2.DATA_ROOT
        assert os.path.isdir(self.data_root), f"{self.data_root} does not exists"
        # File list
        pfilelist = os.path.join(self.data_root, 'frames_list.json')
    ...
    def get_fpaths(self, index):
        ...
        vname = self._video_names[index]
        if file_list is not None and vname in file_list:
            allframes = self.file_list[vname]
        else:
    ...
    def get_seq_frames(self, index, video_length):
        ...
        """
        idx = random.randint(0, video_length - 1)
        return [idx]
    ...
    def __getitem__(self, index):
        ...
        fpaths, boxes, contact_state = self.get_boxes(index)
    ...
    def _aug_frame(
        ...
        list_img, boxes, _ = aug_transform(
            list_img,
            boxes=boxes,
        )
    ...
    def get_boxes(self, index):
        ...
        :return:
        """
        boxes_path = f"{self.data_root}/bbox_jsons"
        self.coord_nr_frames = 1
        self.num_boxes = self.cfg.SVIT.O

        # vid_id = self.json_data[index].id
        vid_id = self._video_names[index]

        folder_id = str(int(self._video_names[index]))
        json_path = '{}/{}.json'.format(boxes_path, folder_id)
        ...
        for frame_index, frame_id in enumerate(coord_frame_list):
            ...
            for box_data in frame_data['labels']:
                cat = 'hand' if box_data['standard_category'] == 'hand' else 'obj'
                if inds[cat] > 1: continue
                global_box_id = inds[cat] + offsets[cat]
                inds[cat] += 1
        ...
        box_tensors, contact_state = box_ops.match_haog(box_tensors)
        return frames, box_tensors, contact_state
