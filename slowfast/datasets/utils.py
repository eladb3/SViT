...
def spatial_sampling(
    ...
    if spatial_idx == -1:
        if aspect_ratio is None and scale is None:
            ...
            frames, boxes = transform.random_crop(frames, crop_size, boxes=boxes, rand_params = rand_params)
        else:
            if isinstance(motion_shift, str):
                transform_func = getattr(transform, motion_shift)
            else:
                transform_func = (
        ...
        if random_horizontal_flip:
            frames, boxes = transform.horizontal_flip(0.5, frames, boxes=boxes, rand_params = rand_params)
    else:
...
def _init_fn(idx):
    try:
        ...
        torch.cuda.set_device(f"cuda:{idx%8}")
